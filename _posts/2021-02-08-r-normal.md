...
`dnorm` gives the PDF density, `pnorm` gives the CDF cumulative distribution function, `qnorm` gives the quantile function, and `rnorm` generates random deviates.
---
...
**Example**: Combine two random variables
...
**Example**: Calculate the $\sigma$ interval percentage
