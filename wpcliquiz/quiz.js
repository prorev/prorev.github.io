...
var ELEMENT_LIST    = 'cache,cap,cli,comment,core,cron,db,eval,eval-file,export,help,import,media,menu,network,option,package,plugin,post,post-type,rewrite,role,scaffold,search-replace,server,shell,sidebar,site,super-admin,taxonomy,term,theme,transient,user,widget';
var one_second      = 1000;
...
var intro_html  = '<div id="intro"><h2>How To Play</h2><p>On the next screen, enter as many wpcli '+what+' as you can think of. For instance "help" is one command. Correct answers will automatically be logged as you type within the input field. Once your time is up, any '+what+' you missed will be listed so you can improve for next time!</p><button type="button" id="start_button" title="Go">Start</button></div>';
...
var outro_html  = '<div id="outro"><h2>Finished!</h2><p>You named <strong id="named">0</strong> wpcli '+what+' in ' + min + ' minutes!</p><div id="share"><h2>Share Your Score</h2></div><p id="missed_message">You missed the following '+what+':</p><ul id="missed_elements" class="element_list"></ul><button type="button" id="replay">Again?</button>';
...
function stopQuiz() {
    var solved_elements = parseInt(solved.children().length),
        share_text      = 'I was able to name ' + solved_elements + ' wpcli '+what+' in ' + min + ' minutes!',
...
function trackEvent(event_type) {
    if (ga) {        
        ga('send', 'event', 'Quiz', event_type, 'label', 1);
    }
}

function trackScore(score) {
    if (ga) {        
        ga('send', 'event', 'Score', 'Points', 'label', score);
    }
}
